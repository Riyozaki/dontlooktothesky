...
- `images/` — фоны, персонажи и референсы; исходные UI-листы для нарезки — в `images/ui_source/`;
- `game/audio/music/` — вся музыка (53 трека), подключается из `definitions.rpy` через `audio/music/...`;
- `game/audio/sfx/` — подключённые звуки (Kenney CC0, см. `docs/licenses/`);
- `game/ui/` — используемые интерфейсные ассеты Ren'Py;
